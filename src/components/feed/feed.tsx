                            ...
                            >
                                ...
                                <div className={`${feedStyles.feedInner}`}>
                                    <div className={`${feedStyles.inner}`}>
                                        <>
                                            {ingredientsInfo(item.ingredients).slice(0, ingredientsInfo(item.ingredients).length >= 6 ? 6 : ingredientsInfo(item.ingredients).length).map((ingredient, key) => (<div key={ingredient._id} className={`${feedStyles.pic} ${ingredientsInfo(item.ingredients).length >= 6 && ((key + 1) === (ingredientsInfo(item.ingredients).slice(0, ingredientsInfo(item.ingredients).length >= 6 ? 6 : ingredientsInfo(item.ingredients).length).length)) ? feedStyles.picLess : ''}`} style={{ zIndex: ingredientsInfo(item.ingredients).length - key }}>
